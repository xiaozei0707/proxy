...
var READY_URL_PATTERNS = [
  ...
  /\/(?:eapi|weapi|api)\/user\/account\/get/,
  /\/(?:eapi|weapi|api)\/user\/detail/,
  /\/(?:eapi|weapi|api)\/user\/playlist/,
  /\/(?:eapi|weapi|api)\/v1\/user\/account/,
  /\/(?:eapi|weapi|api)\/login/,
  /\/(?:eapi|weapi|api)\/login\/status/,
  /\/(?:eapi|weapi|api)\/vipauth/,
];
...
function captureCookie(WYYHeaders) {
  ...
  var requestCookie = pickHeader(WYYHeaders, "cookie");

  if (!requestCookie || requestCookie.length < 500) {
...
function isReadyCaptureUrl(url) {
  for (var i = 0; i < READY_URL_PATTERNS.length; i++) {
    if (READY_URL_PATTERNS[i].test(url)) {
      return true;
    }
  }
  return false;
...
function shouldNotify() {
  var now = Date.now();
  var last = Number(
    $prefs.valueForKey(KEY_LAST_NOTIFY) || 0
  );
