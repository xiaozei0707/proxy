...
async function sign() {
  ...
  var reportResults = [];
  reportResults.push(await doDailyTask(user.heybox_id, cookie, "shareArticle"));
  reportResults.push(await doDailyTask(user.heybox_id, cookie, "shareGameDetail"));
  reportResults.push(await doDailyTask(user.heybox_id, cookie, "shareGameComment"));
  ...
  if (isSignedResult(res) || isTaskSigned(task) || isAlreadyDone(res)) {
    notifySigned(result, task, msg, isIgnoreState(res) ? "已经签到了" : "已签到", dailyResults, shareResults, reportResults);
  } else {
...
async function doShareTasks(heyboxId, cookie, headers) {
  ...
  var result = [];
  result.push(await replaySavedGet(KEY_SHARE_NORMAL, heyboxId, sharedHeaders, "帖子"));
  result.push(await replaySavedGet(KEY_SHARE_GAME, heyboxId, sharedHeaders, "详情"));
  result.push(await replaySavedGet(KEY_COMMENT_SHARE, heyboxId, sharedHeaders, "评价"));
  return result;
}

async function replaySavedGet(key, heyboxId, headers, label) {
  var url = $.getdata(key);
  if (!url) return { label: label, ok: false, msg: "未捕获接口" };
  var resp = parseBody(await get(url, headers));
  if (resp.status === "ok" && (resp.msg === "" || resp.msg === undefined)) {
    return { label: label, ok: true, msg: resp.msg || resp.status || "ok" };
  }
...
function notifySigned(result, task, fallbackMsg, subtitle, dailyResults, shareResults, reportResults) {
  result = result || {};
  task = task || {};
  ...
  if (dailyResults && countOk(dailyResults) < dailyResults.length) {
    detail.push("分享接口：" + formatAttemptResults(shareResults));
    detail.push("上报接口：" + formatAttemptResults(reportResults));
  }
...
function formatAttemptResults(list) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    text.push(label + (item.ok ? "成功" : "失败") + (item.msg ? "(" + trimText(item.msg, 12) + ")" : ""));
  }
  return text.join("/");
}
