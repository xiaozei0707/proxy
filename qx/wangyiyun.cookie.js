...
function injectHeaders(headers, cookie) {
  ...
  setHeader(headers, "cookie", finalCookie);
  if (cookies.MUSIC_U) setHeader(headers, "x-music-u", cookies.MUSIC_U);
  if (cookies.__csrf) setHeader(headers, "x-csrf", cookies.__csrf);
  if (cookies.appkey) setHeader(headers, "x-appkey", cookies.appkey);
  if (cookies.appver) setHeader(headers, "x-appver", cookies.appver);
  if (cookies.buildver) setHeader(headers, "x-buildver", cookies.buildver);
  if (cookies.deviceId) setHeader(headers, "x-deviceId", cookies.deviceId);
  if (cookies.sDeviceId) setHeader(headers, "x-sDeviceId", cookies.sDeviceId);
  if (cookies.idfa) setHeader(headers, "x-idfa", cookies.idfa);
  if (cookies.idfv) setHeader(headers, "x-idfv", cookies.idfv);
  if (cookies.machineid) setHeader(headers, "x-machineid", cookies.machineid);
  if (cookies.os) setHeader(headers, "x-os", cookies.os);
  if (cookies.osver) setHeader(headers, "x-osver", cookies.osver);
  if (cookies.URS_APPID) setHeader(headers, "x-urs-appid", cookies.URS_APPID);
  if (cookies.NMDI) setHeader(headers, "x-nmdi", cookies.NMDI);
  if (cookies.caid) setHeader(headers, "x-caid", cookies.caid);
  if (cookies.channel) setHeader(headers, "x-channel", cookies.channel);
  if (cookies.packageType) setHeader(headers, "x-packageType", cookies.packageType);
  if (cookies.NMCID) setHeader(headers, "x-nmcid", cookies.NMCID);
  if (cookies.NMTID) setHeader(headers, "x-nmtid", cookies.NMTID);
  if (mconfig) setHeader(headers, "MConfig-Info", mconfig);
