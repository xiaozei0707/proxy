...
function injectHeaders(headers, cookie) {
  ...
  setHeaderForce(headers, "Cookie", finalCookie);
  setHeaderForce(headers, "cookie", finalCookie);
  if (cookies.MUSIC_U) {
    setHeaderForce(headers, "x-music-u", cookies.MUSIC_U);
    setHeaderForce(headers, "X-Music-U", cookies.MUSIC_U);
    $.setdata(maskValue(cookies.MUSIC_U), KEY_LAST_INJECTED_MUSIC_U);
  }
  if (cookies.__csrf) setHeaderForce(headers, "x-csrf", cookies.__csrf);
  if (cookies.appkey) setHeaderForce(headers, "x-appkey", cookies.appkey);
  if (cookies.appver) setHeaderForce(headers, "x-appver", cookies.appver);
  if (cookies.buildver) setHeaderForce(headers, "x-buildver", cookies.buildver);
  if (cookies.deviceId) setHeaderForce(headers, "x-deviceId", cookies.deviceId);
  if (cookies.sDeviceId) setHeaderForce(headers, "x-sDeviceId", cookies.sDeviceId);
  if (cookies.idfa) setHeaderForce(headers, "x-idfa", cookies.idfa);
  if (cookies.idfv) setHeaderForce(headers, "x-idfv", cookies.idfv);
  if (cookies.machineid) setHeaderForce(headers, "x-machineid", cookies.machineid);
  if (cookies.os) setHeaderForce(headers, "x-os", cookies.os);
  if (cookies.osver) setHeaderForce(headers, "x-osver", cookies.osver);
  if (cookies.URS_APPID) setHeaderForce(headers, "x-urs-appid", cookies.URS_APPID);
  if (cookies.NMDI) setHeaderForce(headers, "x-nmdi", cookies.NMDI);
  if (cookies.caid) setHeaderForce(headers, "x-caid", cookies.caid);
  if (cookies.channel) setHeaderForce(headers, "x-channel", cookies.channel);
  if (cookies.packageType) setHeaderForce(headers, "x-packageType", cookies.packageType);
  if (cookies.NMCID) setHeaderForce(headers, "x-nmcid", cookies.NMCID);
  if (cookies.NMTID) setHeaderForce(headers, "x-nmtid", cookies.NMTID);
  if (mconfig) setHeaderForce(headers, "MConfig-Info", mconfig);
  if (userAgent) setHeaderForce(headers, "User-Agent", userAgent);
}
...
function setHeaderForce(headers, name, value) {
