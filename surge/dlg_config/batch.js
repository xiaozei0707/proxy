...
try {
  ...
  for (var i = 0; i < responses.length; i++) {
    ...
    if (Array.isArray(innerBody.shopItems) || Array.isArray(innerBody.subscriptionConfigs)) {
      shouldPatch = true;
    }
    if (innerBody.trackingProperties && Object.prototype.hasOwnProperty.call(innerBody.trackingProperties, "monetizable_status")) {
      shouldPatch = true;
    }
    if (Object.prototype.hasOwnProperty.call(innerBody, "subscriberLevel")) {
      shouldPatch = true;
    }
    ...
    for (var k = 0; k < innerBody.subscriptionConfigs.length; k++) {
      if (
        innerBody.subscriptionConfigs[k] &&
        (
          innerBody.subscriptionConfigs[k].itemType === "gold_subscription" ||
          Array.isArray(innerBody.subscriptionConfigs[k].subscriptionConfigs)
        )
      ) {
