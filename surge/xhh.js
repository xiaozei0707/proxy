...
function capture() {
  ...
  var hkey = cookies.hkey || getHeader(headers, "hkey");

  if (!pkey || !tokenid) {
    $.msg("小黑盒", "获取失败", "未找到 pkey 或登录令牌，请确认 Surge 捕获脚本已取消注释，并打开小黑盒任务/签到页");
    return;
  }

  var cleanCookie = "pkey=" + pkey + ";x_xhh_tokenid=" + tokenid;
  if (hkey) cleanCookie += ";hkey=" + hkey;
  ...
  var user = {
    ...
    user_agent: getHeader(headers, "user-agent") || "xiaoheihe/1.3.388 (com.max.xiaoheihe; iOS) Alamofire/5.9.0",
  };

  var saved =
    $.setdata(cleanCookie, KEY_COOKIE) &&
    $.setdata(tokenid, KEY_TOKEN) &&
    $.setdata(pkey, KEY_PKEY) &&
  ...
  if (!saved) {
    $.msg("小黑盒", "保存失败", "Surge 持久化写入失败，请检查脚本权限或重新加载模块");
    return;
  }
