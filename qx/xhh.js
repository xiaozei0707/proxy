...
async function sign() {
  ...
  var dailyResults = [];
  dailyResults.push(await doDailyTask(user.heybox_id, cookie, "shareArticle"));
  dailyResults.push(await doDailyTask(user.heybox_id, cookie, "shareGameDetail"));
  dailyResults.push(await doDailyTask(user.heybox_id, cookie, "shareGameComment"));
...
function notifySigned(result, task, fallbackMsg, subtitle, dailyResults) {
  ...
  if (dailyResults && dailyResults.length) detail.push("分享任务 " + countOk(dailyResults) + "/" + dailyResults.length);
