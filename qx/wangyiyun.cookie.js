...
function main() {
  ...
  if (savedCookie) {
    injectSavedAccount(WYYHeaders, savedCookie);

    var savedMconfig = read(KEY_MCONFIG);
    var savedUserAgent = read(KEY_USER_AGENT);
    if (savedMconfig) setHeader(WYYHeaders, "mconfig-info", savedMconfig);
    if (savedUserAgent) setHeader(WYYHeaders, "user-agent", savedUserAgent);
  ...
  injectSavedAccount(WYYHeaders, cookie);
  if (mconfig) setHeader(WYYHeaders, "mconfig-info", mconfig);
  if (userAgent) setHeader(WYYHeaders, "user-agent", userAgent);
...
function injectSavedAccount(headers, cookie) {
  var fields = parseCookie(cookie);
  setHeader(headers, "cookie", cookie);
  setHeaderFromCookie(headers, fields, "x-music-u", "MUSIC_U");
  setHeaderFromCookie(headers, fields, "x-caid", "caid");
  setHeaderFromCookie(headers, fields, "x-buildver", "buildver");
  setHeaderFromCookie(headers, fields, "x-sdeviceid", "sDeviceId");
  setHeaderFromCookie(headers, fields, "x-channel", "channel");
  setHeaderFromCookie(headers, fields, "x-idfa", "idfa");
  setHeaderFromCookie(headers, fields, "x-packagetype", "packageType");
  setHeaderFromCookie(headers, fields, "x-appver", "appver");
  setHeaderFromCookie(headers, fields, "x-deviceid", "deviceId");
  setHeaderFromCookie(headers, fields, "x-os", "os");
  setHeaderFromCookie(headers, fields, "x-osver", "osver");
  setHeaderFromCookie(headers, fields, "x-machineid", "machineid");
  setHeaderFromCookie(headers, fields, "x-appkey", "appkey");
  setHeaderFromCookie(headers, fields, "x-idfv", "idfv");
  setHeaderFromCookie(headers, fields, "x-urs-appid", "URS_APPID");
}

function setHeaderFromCookie(headers, fields, headerName, cookieName) {
  var value = fields[cookieName];
  if (value) setHeader(headers, headerName, value);
}
