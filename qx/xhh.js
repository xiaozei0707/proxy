...
function capture() {
  ...
  if (path === "/task/shared/" || path === "/task/shared") {
    if (query.shared_type === "game") saveCapturedShare(KEY_SHARE_GAME, url, headers, cookies);
    if (query.shared_type === "normal") saveCapturedShare(KEY_SHARE_NORMAL, url, headers, cookies);
  }
  if (path === "/game/comment/share_data/" || path === "/game/comment/share_data") saveCapturedShare(KEY_COMMENT_SHARE, url, headers, cookies);
...
function saveCapturedShare(key, url, headers, cookies) {
  var record = {
    url: url,
    hkey: cookies.hkey || "",
    user_agent: headers["User-Agent"] || headers["user-agent"] || "",
    accept: headers.Accept || headers.accept || "*/*",
    accept_language: headers["Accept-Language"] || headers["accept-language"] || "zh-Hans-CN;q=1.0, en-CN;q=0.9",
    accept_encoding: headers["Accept-Encoding"] || headers["accept-encoding"] || "gzip, deflate",
    referer: headers.Referer || headers.referer || "http://api.maxjia.com/",
    captured_at: Math.floor(Date.now() / 1000),
  };
  $.setdata(JSON.stringify(record), key);
}
...
async function sign() {
  ...
  var shareResults = await doShareTasks(user.heybox_id, cookie, headers);
  ...
  if (isSignedResult(res) || isTaskSigned(task) || isAlreadyDone(res)) {
    notifySigned(result, task, msg, isIgnoreState(res) ? "已经签到了" : "已签到", dailyResults, shareResults);
  } else {
...
async function doShareTasks(heyboxId, cookie, headers) {
  var result = [];
  result.push(await replaySavedGet(KEY_SHARE_NORMAL, heyboxId, cookie, headers, "帖子"));
  result.push(await replaySavedGet(KEY_SHARE_GAME, heyboxId, cookie, headers, "详情"));
  result.push(await replaySavedGet(KEY_COMMENT_SHARE, heyboxId, cookie, headers, "评价"));
  return result;
}

async function replaySavedGet(key, heyboxId, cookie, fallbackHeaders, label) {
  var record = parseShareRecord($.getdata(key));
  if (!record.url) return { label: label, ok: false, msg: "未捕获接口" };
  var finalUrl = replaceQuery(record.url, { heybox_id: heyboxId });
  var headers = buildCapturedShareHeaders(record, cookie, fallbackHeaders);
  var resp = parseBody(await get(finalUrl, headers));
  return { label: label, ok: resp.status === "ok" && (resp.msg === "" || resp.msg === undefined), msg: resp.msg || resp.status || brief(resp) };
}

function parseShareRecord(text) {
  if (!text) return {};
  var data = safeJson(text, null);
  if (data && data.url) return data;
  return { url: text };
}

function buildCapturedShareHeaders(record, cookie, fallbackHeaders) {
  var current = parseCookie(cookie);
  var hkey = record.hkey || parseQuery(record.url).hkey || current.hkey || "";
  var shareCookie = "pkey=" + (current.pkey || "") + ";x_xhh_tokenid=" + (current.x_xhh_tokenid || "");
  if (hkey) shareCookie += ";hkey=" + hkey;
  return {
    Host: "api.xiaoheihe.cn",
    Accept: record.accept || "*/*",
    "Accept-Encoding": record.accept_encoding || "gzip, deflate",
    "Accept-Language": record.accept_language || "zh-Hans-CN;q=1.0, en-CN;q=0.9",
    Connection: "keep-alive",
    Referer: record.referer || "http://api.maxjia.com/",
    "User-Agent": record.user_agent || (fallbackHeaders && fallbackHeaders["User-Agent"]) || "xiaoheihe/1.3.388 (com.max.xiaoheihe; iOS) Alamofire/5.9.0",
    Cookie: shareCookie,
  };
}
...
function notifySigned(result, task, fallbackMsg, subtitle, dailyResults, shareResults) {
  result = result || {};
  task = task || {};
  ...
  if (dailyResults && countOk(dailyResults) < dailyResults.length) {
    if (allUncaptured(shareResults)) {
      detail.push("未捕获分享接口：更新重写后手动分享并回跳一次");
    } else if (hasVerifyError(shareResults)) {
      detail.push("分享校验已失效：重新分享三项并捕获接口");
    } else {
      detail.push("分享请求：" + formatAttemptResults(shareResults));
      detail.push("未完成原因：小黑盒未结算分享任务");
    }
  }
...
function formatAttemptResults(list) {
  if (!list || !list.length) return "无记录";
  var text = [];
  for (var i = 0; i < list.length; i++) {
    var item = list[i] || {};
    var label = item.label || taskLabel(item.name) || String(i + 1);
    text.push(label + (item.ok ? "请求ok" : "失败") + (item.msg ? "(" + trimText(item.msg, 12) + ")" : ""));
  }
  return text.join("/");
}

function allUncaptured(list) {
  if (!list || !list.length) return true;
  for (var i = 0; i < list.length; i++) {
    if (!list[i] || list[i].msg !== "未捕获接口") return false;
  }
  return true;
}

function hasVerifyError(list) {
  if (!list || !list.length) return false;
  for (var i = 0; i < list.length; i++) {
    if (list[i] && /验证参数错误|系统时间不正确|hkey|签名/.test(String(list[i].msg || ""))) return true;
  }
  return false;
}

function taskLabel(name) {
  if (name === "shareArticle") return "帖子";
  if (name === "shareGameDetail") return "详情";
  if (name === "shareGameComment") return "评价";
  return "";
}
