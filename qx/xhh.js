...
async function sign() {
  ...
  var detail = [];
  if (result.sign_in_coin !== undefined) detail.push(result.sign_in_coin + " H币");
  if (result.sign_in_exp !== undefined) detail.push(result.sign_in_exp + " 经验");

  if (res.status === "ok" || after.status === "ok") {
    $.msg("小黑盒签到", msg || "签到完成", detail.join("，"));
  } else {
    $.msg("小黑盒签到", "签到失败", msg || "签到参数可能已过期");
  }
}
